...
/**
   ...
   @see _sift_features()
*/
struct feature* sift_features( IplImage* img, struct feature* feat,int *count )
{
  return _sift_features( img, feat, SIFT_INTVLS, SIFT_SIGMA, SIFT_CONTR_THR,
			 SIFT_CURV_THR, SIFT_IMG_DBL, SIFT_DESCR_WIDTH,
			 SIFT_DESCR_HIST_BINS, count);
}
...
/**
   ...
   @see sift_keypoints()
*/
struct feature* _sift_features( IplImage* img, struct feature* feat, int intvls,
		    ...
		    int img_dbl, int descr_width, int descr_hist_bins,int* count )
{
 ...
 // FILE* file = fopen ("D:\\sift.txt", "wb");
//ifstream transform_file("D:\\sift.txt",ios::in);
...
//    fatal_error( "NULL pointer error, %s, line %d",  __FILE__, __LINE__ );
  ...
  n = features->total;
  feat =(feature*) calloc( n, sizeof(struct feature) );  //int **a=NULL;*a=(int*)calloc(10,sizeof(int));    
  feat =(feature*) cvCvtSeqToArray( features, feat, CV_WHOLE_SEQ );
  /*for( i = 0; i < n; i++ )
    { cout<<"x="<<feat[i].x<<"  y="<<feat[i].y<<"  data="<<feat[i].feature_data<<"  scl="<<feat[i].scl<<"  ori="<<feat[i].ori<<endl;
     fprintf(file,"%lf %lf %p %lf %lf\r\n",feat[i].x,feat[i].y,feat[i].feature_data,feat[i].scl,feat[i].ori);
 	 fflush(file);
				   ...
				   CvMemStorage* storage )
{
  ...
  for( o = 0; o < octvs; o++ )
	...
	for(c = SIFT_IMG_BORDER; c < dog_pyr[o][0]->width-SIFT_IMG_BORDER; c++)
	  ...
	  if( abs( cvGetReal2D( dog_pyr[o][i], r, c ) ) > prelim_contr_thr )
...
{
  double val = cvGetReal2D( dog_pyr[octv][intvl], r, c );
  int i, j, k;
  ...
  if( val > 0 )
	...
	for( j = -1; j <= 1; j++ )
	  for( k = -1; k <= 1; k++ )
	    if( val < cvGetReal2D( dog_pyr[octv][intvl+i], r + j, c + k ) )
	      return 0;
    }
...
static CvMat* deriv_3D( IplImage*** dog_pyr, int octv, int intvl, int r, int c )
{
  ...
  dx = ( cvGetReal2D( dog_pyr[octv][intvl], r, c+1 ) -
	 cvGetReal2D( dog_pyr[octv][intvl], r, c-1 ) ) / 2.0;
  dy = ( cvGetReal2D( dog_pyr[octv][intvl], r+1, c ) -
	 cvGetReal2D( dog_pyr[octv][intvl], r-1, c ) ) / 2.0;
  ds = ( cvGetReal2D( dog_pyr[octv][intvl+1], r, c ) -
	 cvGetReal2D( dog_pyr[octv][intvl-1], r, c ) ) / 2.0;
			  ...
			  int c )
{
  ...
  v = cvGetReal2D( dog_pyr[octv][intvl], r, c );
  dxx = ( cvGetReal2D( dog_pyr[octv][intvl], r, c+1 ) + 
	  cvGetReal2D( dog_pyr[octv][intvl], r, c-1 ) - 2 * v );
  dyy = ( cvGetReal2D( dog_pyr[octv][intvl], r+1, c ) +
	  cvGetReal2D( dog_pyr[octv][intvl], r-1, c ) - 2 * v );
  dss = ( cvGetReal2D( dog_pyr[octv][intvl+1], r, c ) +
	  cvGetReal2D( dog_pyr[octv][intvl-1], r, c ) - 2 * v );
  dxy = ( cvGetReal2D( dog_pyr[octv][intvl], r+1, c+1 ) -
	 cvGetReal2D( dog_pyr[octv][intvl], r+1, c-1 ) -
	  cvGetReal2D( dog_pyr[octv][intvl], r-1, c+1 ) +
	  cvGetReal2D( dog_pyr[octv][intvl], r-1, c-1 ) ) / 4.0;
  dxs = ( cvGetReal2D( dog_pyr[octv][intvl+1], r, c+1 ) -
	  cvGetReal2D( dog_pyr[octv][intvl+1], r, c-1 ) -
	  cvGetReal2D( dog_pyr[octv][intvl-1], r, c+1 ) +
	  cvGetReal2D( dog_pyr[octv][intvl-1], r, c-1 ) ) / 4.0;
  dys = ( cvGetReal2D( dog_pyr[octv][intvl+1], r+1, c ) -
	  cvGetReal2D( dog_pyr[octv][intvl+1], r-1, c ) -
	  cvGetReal2D( dog_pyr[octv][intvl-1], r+1, c ) +
	  cvGetReal2D( dog_pyr[octv][intvl-1], r-1, c ) ) / 4.0;
			    ...
			    int c, double xi, double xr, double xc )
{
  ...
  return  cvGetReal2D( dog_pyr[octv][intvl], r, c ) + t[0] * 0.5;
}
...
static int is_too_edge_like( IplImage* dog_img, int r, int c, int curv_thr )
{
  ...
  d = cvGetReal2D(dog_img, r, c);
  dxx = cvGetReal2D( dog_img, r, c+1 ) + cvGetReal2D( dog_img, r, c-1 ) - 2 * d;
  dyy = cvGetReal2D( dog_img, r+1, c ) + cvGetReal2D( dog_img, r-1, c ) - 2 * d;
  dxy = ( cvGetReal2D(dog_img, r+1, c+1) - cvGetReal2D(dog_img, r+1, c-1) -
	  cvGetReal2D(dog_img, r-1, c+1) + cvGetReal2D(dog_img, r-1, c-1) ) / 4.0;
  tr = dxx + dyy;
  det = dxx * dyy - dxy * dxy;
			      ...
			      double* ori )
{
  ...
  if( r > 0  &&  r < img->height - 1  &&  c > 0  &&  c < img->width - 1 )
    {
      dx =  cvGetReal2D( img, r, c+1 ) -  cvGetReal2D( img, r, c-1 );
      dy = cvGetReal2D( img, r-1, c ) -  cvGetReal2D( img, r+1, c );
